...
(def mr-dataset-path "data/sentiment/aclmdb") ;; the MR polarity dataset path
(def num-filter 100)
(def num-label 2)
(def dropout 0.5)
...
(defn get-multi-filter-convnet [num-embed sentence-size batch-size filter-size]
  (let [filter-list [3]
...
(defn train-convnet [{:keys [devs embedding-size batch-size test-size num-epoch max-examples]}]
  (let [ ms-dataset (data-helper/load-ms-with-embeddings mr-dataset-path max-examples embedding-size)
